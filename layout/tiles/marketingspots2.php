...
for ($mspot = 4; $mspot <= 6; $mspot++) {
    $spotimage[$mspot] = \theme_essentialbe\toolbox::get_setting('marketing'.$mspot.'image');
    if ($spotimage[$mspot]) {
        $additionalmarketingclass = true;
    }
    $spotbutton[$mspot] = $OUTPUT->essentialbe_marketing_button($mspot);
    if ($spotbutton[$mspot]) {
...
<div class="row-fluid<?php
    ...
    <div class="row-fluid">
    <?php for ($mspot = 4; $mspot <= 6; $mspot++) {
            echo '<!-- Spot #'.$mspot.' -->'; ?>
        <div class="marketing-spot span4">
            <div class="title"><h5><span>
                <span aria-hidden="true" class="fa fa-<?php
                    echo \theme_essentialbe\toolbox::get_setting('marketing'.$mspot.'icon'); ?>"></span>
                <?php echo \theme_essentialbe\toolbox::get_setting('marketing'.$mspot, true); ?>
            </span></h5></div>
...
?>
            <div class="content<?php echo ($additionalmarketingcontentclass) ? ' withbutton' : ''; ?>">
                <?php
                    echo \theme_essentialbe\toolbox::get_setting('marketing'.$mspot.'content', 'format_html');
