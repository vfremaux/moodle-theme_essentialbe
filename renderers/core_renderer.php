...
class theme_essentialbe_core_renderer extends core_renderer {
    ...
    protected function render_custom_menu_item(custom_menu_item $menunode, $level = 0) {
        ...
        if ($menunode->has_children()) {
            ...
            if ($menunode->get_url() !== null) {
                ...
                if ($url != '') {
                    $url = $this->post_process_url_check_access($url);
                    if (!$url) {
                        return;
                    }
                }
    ...
    */
    ...
    public function custom_menu_courses() {
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            ...
            if ($mycoursetitle == 'module') {
                ...
                $branchtitle = get_string('myunits', 'theme_essentialbe');
            } else if ($mycoursetitle == 'class') {
                $branchtitle = get_string('myclasses', 'theme_essentialbe');
            } else {
            ...
            if ($numcourses == 0 || empty($courses)) {
                $noenrolments = get_string('noenrolments', 'theme_essentialbe');
    ...
    public function custom_menu_themecolours()
    {
        ...
        if (!isguestuser()) {
            ...
            if (!empty($alternativethemes)) {
                $branchtitle = get_string('themecolors', 'theme_essentialbe');
                $branchlabel = '<i class="fa fa-th-large"></i>' . $branchtitle;
                $branchurl = new moodle_url('#');
                $branchsort = 300;
                $branch = $colourmenu->add($branchlabel, $branchurl, $branchtitle, $branchsort);

                $defaultthemecolorslabel = get_string('defaultcolors', 'theme_essentialbe');
                ...
                foreach ($alternativethemes as $alternativethemenumber) {
                    ...
                    $branch->add('<i class="fa fa-square colours-alternative' . $alternativethemenumber . '"></i>' . $alternativethemeslabel,
                        new moodle_url($this->page->url, array('essentialbecolours' => 'alternative' . $alternativethemenumber)), $alternativethemeslabel);
                }
            }
        }
    ...
    public function custom_menu_messages() {
        ...
        if (empty($totalmessages)) {
            ...
            $messagetitle = get_string('nomessagesfound', 'theme_essentialbe');
    ...
    private function get_time_difference($created_time)
    {
        ...
        if (intval($time_difference / $years) > 1) {
            ...
            return get_string('ago', 'core_message', get_string('few', 'theme_essentialbe') . get_string('seconds'));
        }
    }
    ...
    public function custom_menu_user() {
        ...
        if (!isloggedin()) {
            ...
            // Render Help Link
            $usermenu .= $this->theme_essentialbe_render_helplink();
            ...
            if (\core\session\manager::is_loggedinas()) {
                ...
                $branchlabel = '<em><i class="fa fa-key"></i>' . fullname($realuser, true) . get_string('loggedinas', 'theme_essentialbe') . fullname($USER, true) . '</em>';
            ...
            if (is_mnet_remote_user($USER) && $idprovider = $DB->get_record('mnet_host', array('id' => $USER->mnethostid))) {
                $branchlabel = '<em><i class="fa fa-users"></i>' . get_string('loggedinfrom', 'theme_essentialbe') . $idprovider->name . '</em>';
            ...
            $usermenu .= $this->theme_essentialbe_render_preferences($context);
            ...
            if ($course->id == 1) {
                ...
                foreach ($hascourses as $hascourse) {
                    ...
                    if (has_capability('gradereport/user:view', $reportcontext) && $hascourse->visible) {
                        $branchlabel = '<em><i class="fa fa-list-alt"></i>' . get_string('mygrades', 'theme_essentialbe') . '</em>';
                ...
                $branchlabel = '<em><i class="fa fa-list-alt"></i>' . get_string('coursegrades', 'theme_essentialbe') . '</em>';
    ...
    private function theme_essentialbe_render_helplink()
    {
        ...
        if (theme_essentialbe_get_setting('helplinktype') === '1') {
            if (theme_essentialbe_get_setting('helplink') && filter_var(theme_essentialbe_get_setting('helplink'), FILTER_VALIDATE_EMAIL)) {
                ...
                if (is_siteadmin()) {
                    $branchurl = preg_replace("(https?:)", "", $CFG->wwwroot).'/admin/settings.php?section=theme_essentialbe_header';
                }
    ...
    */
     ...
     * @return string HTML.
     */
    public function essentialbe_blocks($region, $classes = array(), $tag = 'aside', $footer = 0) {
        ...
        if ($footer > 0) {
            ...
            $output = html_writer::tag($tag, $this->essentialbe_blocks_for_region($region, $footer, $editing), $attributes);
        } else {
     ...
     */
    protected function essentialbe_blocks_for_region($region, $blocksperrow, $editing) {
