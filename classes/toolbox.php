...
class toolbox {
    ...
    static public function get_csswww() {
        ...
        if (!self::lte_ie9()) {
            ...
            $url = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php",
                            "/$syscontext->id/theme_essentialbe/style/$itemid/$moodlecss");
            $url = preg_replace('|^https?://|i', '//', $url->out(false));
            return '<link rel="stylesheet" href="' . $url . '">';
        } else {
            if (\right_to_left()) {
                $moodlecssone = 'essentialbe-rtl_ie9-blessed1.css';
                $moodlecsstwo = 'essentialbe-rtl_ie9.css';
            } else {
                $moodlecssone = 'essentialbe_ie9-blessed1.css';
                $moodlecsstwo = 'essentialbe_ie9.css';
            }

            $syscontext = \context_system::instance();
            $itemid = \theme_get_revision();
            $urlone = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php",
                            "/$syscontext->id/theme_essentialbe/style/$itemid/$moodlecssone");
            $urlone = preg_replace('|^https?://|i', '//', $urlone->out(false));
            $urltwo = \moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php",
                            "/$syscontext->id/theme_essentialbe/style/$itemid/$moodlecsstwo");
    ...
    static public function course_content_search() {
        $canwe = false;
        global $CFG;
        if ((self::get_setting('coursecontentsearch')) && (file_exists("$CFG->dirroot/theme/essentialbe/"))) {
            $canwe = true;
        }
        return $canwe;
    }
    ...
    static private function check_corerenderer() {
        ...
        if (empty($us->corerenderer)) {
            ...
            global $OUTPUT;
            if (property_exists($OUTPUT, 'essentialbe')) {
                ...
                // Fallback check.
                if (($corerenderer != null) && (property_exists($corerenderer, 'essentialbe'))) {
                    ...
                    if (($corerenderer != null) && (property_exists($corerenderer, 'essentialbe'))) {
                        ...
                        $us->corerenderer = $PAGE->get_renderer('theme_essentialbe', 'core');
                    }
                }
            }
        }
    ...
    static public function render_indicators($numberofslides) {
        ...
        for ($indicatorslideindex = 0; $indicatorslideindex < $numberofslides; $indicatorslideindex++) {
            $indicators .= '<li data-target="#essentialbeCarousel" data-slide-to="'.$indicatorslideindex.'"';
    ...
    static public function render_slide_controls() {
        ...
        $prev = '<a class="left carousel-control" href="#essentialbeCarousel" data-slide="prev" aria-label="'.$strprev.'">';
        $prev .= '<span aria-hidden="true" class="fa fa-chevron-circle-left"></span></a>';
        $next = '<a class="right carousel-control" href="#essentialbeCarousel" data-slide="next" aria-label="'.$strnext.'">';
    ...
    static public function initialise_colourswitcher(\moodle_page $page) {
        ...
        $page->requires->js_call_amd('theme_essentialbe/coloursswitcher', 'init',
