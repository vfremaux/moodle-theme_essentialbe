 ...
 */
class theme_essentialbe_toolbox_testcase extends advanced_testcase {
    ...
    protected function setup_renderer() {
        ...
        $toolbox = \theme_essentialbe\toolbox::get_instance();
    ...
    public function test_get_tilefile_header() {
        $this->setup_renderer();
        $thefile = \theme_essentialbe\toolbox::get_tile_file('header');
        global $CFG;
        $withoutdirroot = str_replace($CFG->dirroot, '', $thefile);

        $this->assertEquals('/theme/essentialbe/layout/tiles/header.php', $withoutdirroot);
    }

    public function test_get_tilefile_pagesettings() {
        $this->setup_renderer();
        $thefile = \theme_essentialbe\toolbox::get_tile_file('pagesettings');
        global $CFG;
        $withoutdirroot = str_replace($CFG->dirroot, '', $thefile);

        $this->assertEquals('/theme/essentialbe/layout/tiles/pagesettings.php', $withoutdirroot);
    }

    public function test_get_tilefile_slideshow() {
        $this->setup_renderer();
        $thefile = \theme_essentialbe\toolbox::get_tile_file('slideshow');
        global $CFG;
        $withoutdirroot = str_replace($CFG->dirroot, '', $thefile);

        $this->assertEquals('/theme/essentialbe/layout/tiles/slideshow.php', $withoutdirroot);
    }

    public function test_render_indicators() {
        $this->setup_renderer();
        $theindicators = \theme_essentialbe\toolbox::render_indicators(4);
        $thecontent = '<li data-target="#essentialbeCarousel" data-slide-to="0" class="active"></li>';
        $thecontent .= '<li data-target="#essentialbeCarousel" data-slide-to="1"></li>';
        $thecontent .= '<li data-target="#essentialbeCarousel" data-slide-to="2"></li>';
        $thecontent .= '<li data-target="#essentialbeCarousel" data-slide-to="3"></li>';
        $this->assertEquals($thecontent, $theindicators);
    }

    public function test_render_slide() {
        set_config('slide1url', 'https://about.me/gjbarnard', 'theme_essentialbe');
        set_config('slide1target', '_blank', 'theme_essentialbe');
        set_config('slide1', 'Test slide one', 'theme_essentialbe');
        set_config('slide1caption', '<p>Test of link in caption: <a href="https://about.me/gjbarnard" target="_blank">me.</a></p>',
            'theme_essentialbe');
        set_config('slide2target', '_blank', 'theme_essentialbe');
        set_config('slide2', 'Test slide two', 'theme_essentialbe');
        ...
        $theslide1 = \theme_essentialbe\toolbox::render_slide(1, 0);
        ...
        $thecontent1 .= '<img src="http://www.example.com/moodle/theme/image.php/_s/essentialbe/theme/1/default_slide" ';
        $thecontent1 .= 'alt="Test slide one" class="carousel-image"></div></div></div></div></a>';
        $this->assertEquals($thecontent1, $theslide1);

        $theslide2 = \theme_essentialbe\toolbox::render_slide(2, 0);
        ...
        $thecontent2 .= '<img src="http://www.example.com/moodle/theme/image.php/_s/essentialbe/theme/1/default_slide" ';
    ...
    public function test_themeinfo() {
        ...
        $this->assertEquals('essentialbe', $PAGE->theme->name);
        $this->assertEquals('/theme/essentialbe', $themedir);
